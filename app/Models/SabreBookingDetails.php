...
class SabreBookingDetails extends Model
{
    ...
    public static function getBookingDetails($pnrId)
    {
        ...
        if ($flightBookingInfo->status == 2) {
            $flightPassengers = FlightPassenger::where('flight_booking_id', $flightBookingInfo->id)->get();
            if (isset($responseData['flightTickets']) && count($responseData['flightTickets']) > 0) {
                $flightPassengerIndex = 0;
                foreach ($flightPassengers as $flightPassenger) {
                    FlightPassenger::where('id', $flightPassenger->id)->update([
                        'ticket_no' => $responseData['flightTickets'][$flightPassengerIndex]['number'] ?? null
                    ]);
                    $flightPassengerIndex++;
                }
            }
        }
        // update ticket no if flight is ticketed
