...
class FlyhubFlightBooking extends Model
{
    ...
    public static function updateTravellers($request, $revalidatedData)
    {
        ...
        foreach ($request->first_name as $passengerIndex => $firstName) {
            ...
            $type = $request->passenger_type[$passengerIndex];
            $title = $request->titles[$passengerIndex];
            $docIssueCountry = DB::table('country')->where('iso3', $request->document_issue_country[$passengerIndex])->first();

            $passenger[] = array(
                "pax_id" => (string) $passengerIndex + 1,
                ...
                "last_name" => (string) $request->last_name[$passengerIndex],
                "dob" => (string) $request->dob[$passengerIndex],
                "doc_country" => (string) $docIssueCountry->iso,
                "doc_no" => (string) $request->document_no[$passengerIndex],
                "doc_dateofexpiry" => (string) $request->document_expire_date[$passengerIndex],
                "doc_dateofissue" => (string) "",
                "frequent_flyer" => (string) $request->frequent_flyer_no[$passengerIndex],
                "isd_code" => (string) "880",
                "contact_no" => (string) substr($request->phone[$passengerIndex], -11),
                "email_address" => (string) $request->email[$passengerIndex],
