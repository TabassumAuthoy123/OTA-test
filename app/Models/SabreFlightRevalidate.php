...
class SabreFlightRevalidate extends Model
{
    ...
    public static function flightRevalidate($sessionIndex)
    {
        ...
        // INF does NOT request a seat
        $seatsRequested = $adult + $child;

        $passengerTypes = [];
        foreach (['ADT' => $adult, 'CNN' => $child, 'INF' => $infant] as $code => $qty) {
            if ((int) $qty > 0) {
                $passengerTypes[] = [
                    "Code" => $code,
                    "Quantity" => (int) $qty,
        ...
        $searchResults = json_decode(session('search_results'), true);
        if (!$searchResults)
            return json_encode(["error" => "search_results invalid"]);

        $group = $searchResults['groupedItineraryResponse']['itineraryGroups'][0] ?? null;
        if (!$group)
            return json_encode(["error" => "Missing itineraryGroups[0]"]);

        $itinerary = $group['itineraries'][$sessionIndex] ?? null;
        if (!$itinerary)
            return json_encode(["error" => "Invalid sessionIndex"]);

        $legsArray = $itinerary['legs'] ?? [];
        $legDescriptions = $group['groupDescription']['legDescriptions'] ?? [];
        $legDescs = $searchResults['groupedItineraryResponse']['legDescs'] ?? [];
        $scheduleDescs = $searchResults['groupedItineraryResponse']['scheduleDescs'] ?? [];

        if (empty($legsArray))
            return json_encode(["error" => "legs empty"]);

        // ---- helpers: NO TZ in outgoing request
        $timeNoZone = function ($t) {
            if (!is_string($t) || $t === '')
                return "00:00:00";
        ...
        $isoNoZone = function (string $date, string $timePart) {
            return substr($date, 0, 10) . "T" . substr($timePart, 0, 8);
        };

        // ---- booking codes flat
        $pi0 = $itinerary['pricingInformation'][0] ?? null;
        if (!$pi0)
            return json_encode(["error" => "pricingInformation[0] missing"]);

        $fareComponents = $pi0['fare']['passengerInfoList'][0]['passengerInfo']['fareComponents'] ?? [];
        if (empty($fareComponents))
            return json_encode(["error" => "fareComponents missing"]);

        $bookingCodesFlat = [];
        foreach ($fareComponents as $fc) {
            foreach (($fc['segments'] ?? []) as $seg) {
                $bookingCodesFlat[] = (string) ($seg['segment']['bookingCode'] ?? 'L');
            }
        }
        ...
        foreach ($legsArray as $legKey => $leg) {
            $legRef = ((int) ($leg['ref'] ?? 0)) - 1;
            $legDesc = $legDescs[$legRef] ?? null;
            if (!$legDesc)
                continue;

            $baseDepartureDate = $legDescriptions[$legKey]['departureDate'] ?? null;
            if (!$baseDepartureDate)
                continue;

            $schedulesArray = $legDesc['schedules'] ?? [];
            ...
            foreach ($schedulesArray as $schedule) {
                $scheduleRefIdx = ((int) ($schedule['ref'] ?? 0)) - 1;
                $schedDesc = $scheduleDescs[$scheduleRefIdx] ?? null;
                if (!$schedDesc)
                    continue;

                $depAdj = (int) ($schedule['departureDateAdjustment'] ?? 0);
                $arrAdj = (int) ($schedDesc['arrival']['dateAdjustment'] ?? 0);

                $depDate = date("Y-m-d", strtotime("+" . $depAdj . " day", strtotime($baseDepartureDate)));
                $arrDate = date("Y-m-d", strtotime(($arrAdj >= 0 ? "+" : "") . $arrAdj . " day", strtotime($depDate)));

                $depTime = $timeNoZone($schedDesc['departure']['time'] ?? "");
                $arrTime = $timeNoZone($schedDesc['arrival']['time'] ?? "");

                $depDT = $isoNoZone($depDate, $depTime);
                $arrDT = $isoNoZone($arrDate, $arrTime);

                $flightNumber = (int) ($schedDesc['carrier']['marketingFlightNumber'] ?? 0);
                if ($flightNumber <= 0) {
                    return json_encode(["error" => "Invalid flight number", "carrier" => $schedDesc['carrier'] ?? null]);
                }

                $bookingCode = $bookingCodesFlat[$globalSegIndex] ?? 'L';
                $globalSegIndex++;

                $segObj = [
                    "Airline" => [
                        "Marketing" => (string) ($schedDesc['carrier']['marketing'] ?? ""),
                        "Operating" => (string) ($schedDesc['carrier']['operating'] ?? ($schedDesc['carrier']['marketing'] ?? "")),
                    ],
                    "Number" => $flightNumber,
                    "ClassOfService" => (string) $bookingCode,
                    "OriginLocation" => ["LocationCode" => (string) ($schedDesc['departure']['airport'] ?? "")],
                    "DestinationLocation" => ["LocationCode" => (string) ($schedDesc['arrival']['airport'] ?? "")],
                ...
                $flightInformation[] = $segObj;
            }

            if (empty($flightInformation) || !$firstSeg || !$lastSeg)
                continue;

            $originDestinationInformationArray[] = [
                "RPH" => (string) $rph,
                "DepartureDateTime" => $firstSeg["DepartureDateTime"],
                "OriginLocation" => $firstSeg["OriginLocation"],
                "DestinationLocation" => $lastSeg["DestinationLocation"],
                "TPA_Extensions" => [
                    // ✅ important for connected segments
                    "SegmentType" => ["Code" => "X"],
        ...
        if (empty($originDestinationInformationArray)) {
            return json_encode(["error" => "No ODIs built"]);
        }

        $requestBody = [
            "OTA_AirLowFareSearchRQ" => [
                "Version" => "6.8.0",
                // "AvailableFlightsOnly" => false, // should not use
                "TravelPreferences" => [
                    "TPA_Extensions" => [
                        "VerificationItinCallLogic" => [
                            // ✅ switch to M (commonly used in working revalidate)
                            "Value" => "M",
                            "AlwaysCheckAvailability" => true
                        ],
                    ],
        ...
        $sabreGdsInfo = SabreGdsConfig::find(1);
        $apiEndPoint = $sabreGdsInfo->is_production
            ? 'https://api.platform.sabre.com/v5/shop/flights/revalidate'
            : 'https://api.cert.platform.sabre.com/v5/shop/flights/revalidate';
