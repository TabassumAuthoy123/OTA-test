...
class FlightController extends Controller
{
    public function bookMyFlight(Request $request)
    {
        if ($request->header('Authorization-Header') == AuthenticationController::AUTHORIZATION_TOKEN) {
            ...
            $request->validate([
                ...
                'passenger_data.*.passenger_type' => 'required|string',
    ...
    public function myFlightBookings(Request $request)
    {
        if ($request->header('Authorization-Header') == AuthenticationController::AUTHORIZATION_TOKEN) {
            ...
            $flightBookings = FlightBooking::where('passenger_id', $request->user()->id)->orderBy('id', 'desc')->paginate(10);
    ...
    public function flightBookingDetails(Request $request)
    {
        if ($request->header('Authorization-Header') == AuthenticationController::AUTHORIZATION_TOKEN) {
            ...
            $flightBooking = FlightBooking::where('booking_no', $request->booking_no)->where('passenger_id', $request->user()->id)->first();
            if ($flightBooking) {
                return response()->json([
                    'success' => true,
