...
use App\Http\Requests\SaveMfsAccountRequest;
use App\Http\Requests\UpdateMfsAccountRequest;
use App\Http\Requests\SubmitRechargeRequest;
...
class PaymentController extends Controller
{
    ...
    public function saveBankAccount(SaveBankAccountRequest $request)
    {
    ...
    public function saveMfsAccount(SaveMfsAccountRequest $request)
    {
    ...
    public function updateMfsAccount(UpdateMfsAccountRequest $request)
    {
    ...
    public function createTopupRequest()
    {
        ...
        $b2bUsers = User::where('user_type', UserType::B2B->value)->orderBy('name', 'asc')->get();
        return view('recharge.create', compact('bankAccounts', 'mfsAccounts', 'b2bUsers'));
    }

    public function submitRechargeRequest(SubmitRechargeRequest $request)
    {
    ...
    public function deductB2bAccount(Request $request)
    {
        ...
        try {
            ...
            Log::error('B2B account deduction failed: ' . $e->getMessage(), [
                'b2b_user_id' => $request->b2b_user_id,
                'amount' => $request->amount
            ]);
            Toastr::error('Deduction failed. Please try again.');
            return back();
        }
    }
    ...
    public function success(Request $request)
    {
        ...
        if ($validate) {
            try {
                ...
                Toastr::error('Payment processing failed. Please contact support.');
                return redirect('/view/recharge/requests');
            }
        }
    }
    ...
    public function refund($bankID)
    {
        ...
        if ($refund->status) {
            ...
            return response()->json(['success' => true, 'ref_id' => $refID, 'data' => $refund->output]);
        } else {
    ...
    public function check_refund_status($refID)
    {
        ...
        if ($refund->status) {
            ...
            return response()->json(['success' => true, 'ref_id' => $refID, 'state' => $state, 'data' => $refund->output]);
        } else {
            return $refund->message;
        }
    }
    ...
    public function get_transaction_status($trxID)
    {
        ...
        if ($query->status) {
            return response()->json(['success' => true, 'data' => $query->output]);
        } else {
            return response()->json(['success' => false, 'message' => $query->message]);
        }
    }
