                ...
                <div class="sorting my-3">
                    <div class="row">
                        ...
                        <div class="col-lg-5 text-end">
                            @if(session('flight_type') != 3)
                                @if(session('departure_date') > date("Y-m-d"))
                                    <a href="{{url('search/prev/day')}}" onclick="showLoader()" class="d-inline-block search_prev"><i
                                            class="fas fa-arrow-left"></i> Previous Day</a>
                                @endif
                                <a href="{{url('search/next/day')}}" onclick="showLoader()" class="d-inline-block search_next">Next
                                    Day <i class="fas fa-arrow-right"></i></a>
                            @endif
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-lg-9 mainContent">
                        <div class="theiaStickySidebar">
                            ...
                            <div class="alert alert-primary" style="padding: .5rem 1rem;">
...
@section('footer_js')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            var flightType = "{{session('flight_type')}}";
        ...
        function updateFlightDate() {
            ...
            const depStr = "{{ date('d-m-Y', strtotime(session('departure_date'))) }}" || todayStr;

            // ✅ Only format return date if the session value exists
            const retRaw = "{{ session('return_date') }}";
            const retStr = retRaw
                ? "{{ date('d-m-Y', strtotime(session('return_date'))) }}"
                : null;
            ...
            $(".t-datepicker").tDatePicker({
                ...
                dateCheckIn: depStr,
                dateCheckOut: retStr, // null for one-way
                iconDate: "",
