        ...
        @foreach ($search_results_operating_carriers as $carrierCode)
            ...
            @if ($carrierCodeInfo)
                <div class="custom-control custom-checkbox fs-14 d-flex check-area" style="padding-left: 7px">
                    <input id="airline-{{ $carrierCode }}" onclick="airlineCarrierFilter('{{ $carrierCode }}')" @if(session('airline_carrier_code') && in_array($carrierCode, session('airline_carrier_code'))) checked @endif type="checkbox" value="{{ $carrierCode }}" class="custom-control-input px-2 single-check">
