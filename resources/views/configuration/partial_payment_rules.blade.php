...
<div class="row"><div class="col-lg-12">
  <div class="card" style="border-radius:8px;overflow:hidden;">
    <div class="cfg-header">
      <div>
        <h5><i class="typcn typcn-credit-card me-2"></i> Partial Payment Rules</h5>
        <small>Configuration &rsaquo; Partial Payment Rules</small>
      </div>
    ...
    <form method="GET" action="{{ url('configuration/partial-payment-rules') }}">
      <div class="cfg-filters">
        <div class="fg">
          <label>Search</label>
          <input type="text" name="search" value="{{ request('search') }}" placeholder="Name / Airline..." style="width:240px;">
        </div>
        <div class="fg">
          <label>Status</label>
        ...
        @if(request()->hasAny(['search','filter_status']))
          <div class="fg"><label>&nbsp;</label>
            <a href="{{ url('configuration/partial-payment-rules') }}" class="btn btn-secondary btn-sm" style="height:34px;">Clear</a>
          </div>
        @endif
      </div>
    </form>
    ...
    <div class="table-responsive">
      <table class="table table-bordered cfg-table mb-0">
        <thead>
          <tr>
            <th>#</th><th>Name</th><th>Min Payment %</th><th>Deferred %</th><th>Due Days</th>
            <th>Applicable For</th><th>Airline</th><th>Route</th><th>Status</th><th>Actions</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr>
            ...
            <td><strong>{{ $rule->min_payment_percent }}%</strong></td>
            <td>{{ $rule->max_defer_percent }}%</td>
            <td>{{ $rule->payment_due_days }} days</td>
            <td>{{ ucfirst($rule->applicable_for) }}</td>
            ...
            <td>
              <button class="btn btn-sm btn-warning btn-edit-pp" style="font-size:11px;"
                data-id="{{ $rule->id }}" data-name="{{ $rule->name }}"
                data-minp="{{ $rule->min_payment_percent }}" data-days="{{ $rule->payment_due_days }}"
                data-app="{{ $rule->applicable_for }}" data-airline="{{ $rule->airline_code }}"
                data-rfrom="{{ $rule->route_from }}" data-rto="{{ $rule->route_to }}"
                data-active="{{ $rule->is_active }}" data-notes="{{ $rule->notes }}">Edit</button>
              <form method="POST" action="{{ url('configuration/partial-payment-rules/'.$rule->id) }}" class="d-inline"
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header" style="background:#1a5276;color:#fff;">
        <h5 class="modal-title">Add Partial Payment Rule</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form method="POST" action="{{ url('configuration/partial-payment-rules') }}">
        @csrf
        <div class="modal-body">
          @include('configuration._partial_payment_fields')
        </div>
...
<div class="modal fade" id="editModal" tabindex="-1">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header" style="background:#1a5276;color:#fff;">
        <h5 class="modal-title">Edit Partial Payment Rule</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <form method="POST" id="editForm">
        @csrf @method('PUT')
        <div class="modal-body">
          @include('configuration._partial_payment_fields', ['edit' => true])
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
...
@section('footer_js')
<script>
document.querySelectorAll('.btn-edit-pp').forEach(btn => {
  btn.addEventListener('click', function() {
    const d = this.dataset;
    const f = document.getElementById('editForm');
    f.action = '/configuration/partial-payment-rules/' + d.id;
    f.querySelector('[name=name]').value = d.name;
    f.querySelector('[name=min_payment_percent]').value = d.minp;
    f.querySelector('[name=payment_due_days]').value = d.days;
    f.querySelector('[name=applicable_for]').value = d.app;
