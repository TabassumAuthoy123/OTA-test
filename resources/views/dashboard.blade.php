...
<div class="dashboard-container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            ...
            <p class="text-muted mb-0 fs-14">Welcome back, {{ Auth::user()->name }}. Here's your business overview.</p>
        </div>
        <span class="text-muted fs-13">{{ now()->format('l, d M Y') }}</span>
    </div>

    {{-- ═══ QUICK ACTIONS ═══ --}}
    <div class="quick-actions mb-4">
        <a href="{{ url('/home') }}" class="quick-action-btn primary">
            <i class="typcn typcn-plane-outline"></i> Search Flights
        </a>
        <a href="{{ url('view/all/booking') }}" class="quick-action-btn">
            <i class="typcn typcn-th-list-outline"></i> View Bookings
        </a>
        <a href="{{ url('view/saved/passengers') }}" class="quick-action-btn">
            <i class="typcn typcn-user-outline"></i> Saved Passengers
        </a>
        <a href="{{ url('flight/booking/report') }}" class="quick-action-btn">
        ...
        <div class="col-lg-3 col-md-6">
            <div class="stat-card bg-gradient-primary">
                ...
                <div class="stat-label">Total Bookings
                    @if($bookingChange != 0)
                        <span class="trend-badge {{ $bookingChange > 0 ? 'up' : 'down' }}">
                            {{ $bookingChange > 0 ? '↑' : '↓' }}{{ abs($bookingChange) }}%
                        </span>
                    @else
                        <span class="trend-badge neutral">→ 0%</span>
                    @endif
                </div>
                <div class="stat-sub">Today: {{ $todayBookings }} • Yesterday: {{ $yesterdayBookings }} • Month: {{ $monthBookings }}</div>
            </div>
        </div>
        <div class="col-lg-3 col-md-6">
            <div class="stat-card bg-gradient-success">
                <i class="typcn typcn-ticket stat-icon"></i>
                <div class="stat-value">{{ number_format($issuedTickets) }}</div>
                <div class="stat-label">Issued Tickets</div>
                <div class="stat-sub">{{ $confirmedBookings }} awaiting • {{ $pendingBookings }} pending • {{ $cancelledBookings }} cancelled</div>
            </div>
        </div>
        <div class="col-lg-3 col-md-6">
            <div class="stat-card bg-gradient-warning">
                ...
                <div class="stat-label">Total Revenue
                    @if($revenueChange != 0)
                        <span class="trend-badge {{ $revenueChange > 0 ? 'up' : 'down' }}">
                            {{ $revenueChange > 0 ? '↑' : '↓' }}{{ abs($revenueChange) }}%
                        </span>
                    @endif
                </div>
                <div class="stat-sub">Today: ৳{{ number_format($todayRevenue, 0) }} • Month: ৳{{ number_format($monthRevenue, 0) }}</div>
            </div>
        </div>
        <div class="col-lg-3 col-md-6">
            <div class="stat-card bg-gradient-info">
                ...
                <div class="stat-label">Registered Customers
                    @if($monthBookingChange != 0)
                        <span class="trend-badge {{ $monthBookingChange > 0 ? 'up' : 'down' }}">
                            {{ $monthBookingChange > 0 ? '↑' : '↓' }}{{ abs($monthBookingChange) }}% MoM
                        </span>
                    @endif
                </div>
    ...
    {{-- ═══ STATUS BREAKDOWN + MONTHLY REVENUE ═══ --}}
    <div class="row g-3 mb-4">
        {{-- Booking Status --}}
        <div class="col-lg-5">
            <div class="section-card h-100">
                ...
                <div class="d-flex align-items-center gap-4 flex-wrap">
                    ...
                    <div style="position: relative; width: 160px; height: 160px; flex-shrink: 0;">
                        ...
                        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); text-align: center;">
                            <div style="font-size: 24px; font-weight: 700; color: #333; line-height: 1;">{{ $totalBookings }}</div>
                    ...
                    <div class="status-bar" style="flex: 1;">
                        <div class="status-chip pending">
                            <span class="count">{{ $pendingBookings }}</span> Pending
                        </div>
                        <div class="status-chip confirmed">
                            <span class="count">{{ $confirmedBookings }}</span> Booked
                        </div>
                        <div class="status-chip issued">
                            <span class="count">{{ $issuedTickets }}</span> Ticketed
                        </div>
                        <div class="status-chip cancelled">
                            <span class="count">{{ $cancelledBookings }}</span> Cancelled
                        </div>
                        <div class="status-chip voided">
                            <span class="count">{{ $voidedTickets }}</span> Voided
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center py-2" style="border-bottom: 1px solid #f0f2f5;">
                        ...
                        <div>
                            <span class="badge bg-light text-dark">{{ $route->total_bookings }} bookings</span>
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="section-card h-100">
                ...
                <div class="d-flex justify-content-around mt-3 pt-3" style="border-top: 1px solid #f0f2f5;">
                    <div class="text-center">
                        <div style="font-size: 20px; font-weight: 700; color: #667eea;">{{ number_format($monthlyRevenue->sum('bookings')) }}</div>
                        <div style="font-size: 11px; color: #999; text-transform: uppercase; letter-spacing: 0.5px;">Total Bookings</div>
                    </div>
                    <div class="text-center">
                        <div style="font-size: 20px; font-weight: 700; color: #11998e;">৳{{ number_format($monthlyRevenue->sum('revenue'), 0) }}</div>
                        <div style="font-size: 11px; color: #999; text-transform: uppercase; letter-spacing: 0.5px;">Total Revenue</div>
                    </div>
                    <div class="text-center">
                        <div style="font-size: 20px; font-weight: 700; color: #f5576c;">৳{{ $monthlyRevenue->count() > 0 ? number_format($monthlyRevenue->avg('revenue'), 0) : 0 }}</div>
    ...
    <div class="section-card">
        ...
        <div class="row g-3">
            ...
            @endphp
            @foreach($gdsProviders as $gdsName => $gdsConfig)
                @php
                ...
                <div class="col-lg-6 col-md-6">
                    <div class="gds-card {{ $gdsConfig['class'] }}">
                        <div class="gds-header">
                            <span class="gds-name">{{ $gdsConfig['icon'] }} {{ $gdsName }}</span>
                            <span class="badge bg-light text-dark" style="font-size: 11px;">{{ $gdsTotal }} total</span>
                        </div>
                        <div class="d-flex justify-content-between mb-3">
                            <div class="gds-stat">
                                <div class="gds-stat-value text-success">{{ $gdsSuccess }}</div>
                                <div class="gds-stat-label">Successful</div>
                            </div>
                            <div class="gds-stat">
                                <div class="gds-stat-value" style="color: #667eea;">৳{{ number_format($gdsRevenue, 0) }}</div>
                                <div class="gds-stat-label">Revenue</div>
                            </div>
                            <div class="gds-stat">
                                <div class="gds-stat-value text-danger">{{ $gdsCancelled }}</div>
                        ...
                        <div>
                            <div class="d-flex justify-content-between mb-1">
                                ...
                                <span style="font-size: 11px; font-weight: 700; color: {{ $successRate >= 70 ? '#198754' : ($successRate >= 40 ? '#ffc107' : '#dc3545') }};">{{ $successRate }}%</span>
                            </div>
                            <div class="progress" style="height: 6px; border-radius: 3px;">
                                <div class="progress-bar" style="width: {{ $successRate }}%; border-radius: 3px; background: {{ $successRate >= 70 ? '#198754' : ($successRate >= 40 ? '#ffc107' : '#dc3545') }};"></div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="section-card">
        <div class="section-title d-flex justify-content-between align-items-center">
            ...
            <a href="{{ url('view/all/booking') }}" class="btn btn-sm btn-outline-primary" style="font-size: 12px;">View All</a>
        </div>
        <div class="table-responsive">
            <table class="dashboard-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <a href="{{ url('flight/booking/details/' . $booking->booking_no) }}" class="text-decoration-none fw-bold" style="color: #667eea;">
                                {{ $booking->booking_no }}
                            </a>
                        </td>
                        <td>{{ Str::limit($booking->traveller_name, 25) }}</td>
                        <td>
                        ...
                        <td><span class="badge bg-light text-dark">{{ $booking->gds }}</span></td>
                        <td class="text-end fw-bold">{{ $booking->currency }} {{ number_format($booking->total_fare, 0) }}</td>
                        <td>
                        ...
                        <td class="text-muted">{{ $booking->created_at->format('d M, h:i A') }}</td>
                    </tr>
                    @empty
                    <tr>
...
<script>
    ...
    const revenueChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: {!! json_encode($monthlyRevenue->map(fn($m) => \Carbon\Carbon::createFromFormat('Y-m', $m->month)->format('M Y'))->values()) !!},
            datasets: [
                {
                    label: 'Revenue (৳)',
                ...
                {
                    label: 'Bookings',
                    data: {!! json_encode($monthlyRevenue->pluck('bookings')->values()) !!},
                    type: 'line',
                    borderColor: '#11998e',
