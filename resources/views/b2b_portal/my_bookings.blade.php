...
<div class="row"><div class="col-lg-12">
  <div class="card" style="border-radius:8px;overflow:hidden;">
    <div class="b2b-page-header">
      <div>
        <h5><i class="fas fa-ticket-alt me-2"></i> {{ $title }}</h5>
        <small>Dashboard &rsaquo; My Bookings &rsaquo; {{ $title }}</small>
      </div>
    </div>
    ...
    <div class="card-body p-0">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style="font-weight:700;color:#0f1f3d;">
                <a href="{{ url('flight/booking/details/'.$b->booking_no) }}" style="color:#0f1f3d;text-decoration:none;">{{ $b->booking_no }}</a>
              </td>
              <td>{{ $b->pnr_id ?? 'N/A' }}</td>
              <td>
      ...
      <div class="d-flex justify-content-between align-items-center px-3 py-2">
        <small class="text-muted">Showing {{ $bookings->firstItem() ?? 0 }}&ndash;{{ $bookings->lastItem() ?? 0 }} of {{ $bookings->total() }} entries</small>
        {{ $bookings->links() }}
      </div>
    </div>
  </div>
