@extends('master')

@section('header_css')
<style>
.b2b-page-header { background: linear-gradient(135deg,#1e3a5f,#2d5f8a); color:#fff; padding:16px 24px; border-radius:8px 8px 0 0; display:flex; justify-content:space-between; align-items:center; }
.b2b-page-header h5 { margin:0; font-size:18px; font-weight:700; }
.b2b-filters { background:#f8f9fa; padding:14px 16px; border-bottom:1px solid #dee2e6; display:flex; flex-wrap:wrap; gap:10px; align-items:flex-end; }
.b2b-filters .filter-group { display:flex; flex-direction:column; gap:4px; }
.b2b-filters label { font-size:11px; font-weight:600; color:#555; margin:0; }
.b2b-filters input, .b2b-filters select { font-size:13px; padding:5px 10px; border:1px solid #ced4da; border-radius:5px; height:34px; }
.b2b-table th { background:#1e3a5f; color:#fff; font-size:13px; padding:10px 12px; white-space:nowrap; }
.b2b-table td { font-size:13px; padding:9px 12px; vertical-align:middle; }
.b2b-table tr:hover td { background:#f0f4ff; }
.badge-status { padding:4px 10px; border-radius:12px; font-size:11px; font-weight:600; }
.s-booking-request { background:#fff3cd; color:#856404; }
.s-booked { background:#d4edda; color:#155724; }
.s-issued { background:#cce5ff; color:#004085; }
.s-cancelled { background:#f8d7da; color:#721c24; }
.s-ticket-cancelled { background:#e2e3e5; color:#383d41; }
.badge-partial-yes { background:#28a745; color:#fff; padding:3px 8px; border-radius:10px; font-size:11px; }
.badge-partial-no { background:#6c757d; color:#fff; padding:3px 8px; border-radius:10px; font-size:11px; }
.btn-view { background:#17a2b8; color:#fff; border:none; padding:4px 12px; border-radius:5px; font-size:12px; }
.btn-excel { background:#28a745; color:#fff; border:none; padding:6px 14px; border-radius:5px; font-size:13px; font-weight:600; }
.pagination { margin:0; }
</style>
@endsection

@section('content')
<div class="row">
  <div class="col-lg-12">
    <div class="card" style="border-radius:8px; overflow:hidden;">
      <div class="b2b-page-header">
        <h5><i class="typcn typcn-plane-outline me-2"></i> B2B Flight Bookings</h5>
        <small>Dashboard &rsaquo; B2B &rsaquo; Flight Bookings</small>
      </div>

      {{-- Filters --}}
      <form method="GET" action="{{ url('b2b/flight-bookings') }}">
        <div class="b2b-filters">
          <div class="filter-group">
            <label>Search</label>
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Booking ref / PNR / Agency..." style="width:220px;">
          </div>
          <div class="filter-group">
            <label>Status</label>
            <select name="status">
              <option value="all" {{ request('status','all')=='all'?'selected':'' }}>All</option>
              <option value="0" {{ request('status')=='0'?'selected':'' }}>Booking Request</option>
              <option value="1" {{ request('status')=='1'?'selected':'' }}>Booked</option>
              <option value="2" {{ request('status')=='2'?'selected':'' }}>Ticket Issued</option>
              <option value="3" {{ request('status')=='3'?'selected':'' }}>Booking Cancelled</option>
              <option value="4" {{ request('status')=='4'?'selected':'' }}>Ticket Cancelled</option>
            </select>
          </div>
          <div class="filter-group">
            <label>Start Date</label>
            <input type="date" name="start_date" value="{{ request('start_date') }}">
          </div>
          <div class="filter-group">
            <label>End Date</label>
            <input type="date" name="end_date" value="{{ request('end_date') }}">
          </div>
          <div class="filter-group" style="justify-content:flex-end;">
            <label>&nbsp;</label>
            <button type="submit" class="btn btn-primary btn-sm" style="height:34px;">Search</button>
          </div>
          <div class="filter-group" style="margin-left:auto; justify-content:flex-end;">
            <label>&nbsp;</label>
            <a href="{{ url('b2b/flight-bookings') }}?{{ http_build_query(array_merge(request()->all(), ['export'=>'excel'])) }}" class="btn-excel">Export to Excel</a>
          </div>
        </div>
      </form>

      {{-- Table --}}
      <div class="card-body p-0">
        <div class="table-responsive">
          <table class="table table-bordered mb-0 b2b-table">
            <thead>
              <tr>
                <th>SL</th>
                <th>Booking Reference</th>
                <th>Created Date</th>
                <th>Create By</th>
                <th>Agency Name</th>
                <th>GDS PNR</th>
                <th>Airline PNR</th>
                <th>Journey Type</th>
                <th>Status</th>
                <th>Amount</th>
                <th>Partial Payment</th>
                <th>Action</th>
              </tr>
            </thead>
            <tbody>
              @forelse($bookings as $i => $b)
              <tr>
                <td>{{ $bookings->firstItem() + $i }}</td>
                <td><a href="{{ url('flight/booking/details/'.$b->booking_no) }}" style="color:#1e3a5f; font-weight:600;">{{ $b->booking_no }}</a></td>
                <td>{{ $b->created_at ? date('d-M-Y / h:i A', strtotime($b->created_at)) : 'N/A' }}</td>
                <td>{{ $b->booked_by_name }}</td>
                <td>{{ $b->agency_name }}</td>
                <td>{{ $b->pnr_id ?? 'N/A' }}</td>
                <td>{{ $b->airlines_pnr ?? 'N/A' }}</td>
                <td>{{ \App\Http\Controllers\B2bController::journeyTypeLabel($b->flight_type) }}</td>
                <td>
                  @php $sl = \App\Http\Controllers\B2bController::bookingStatusLabel($b->status); @endphp
                  @if($b->status==0)<span class="badge-status s-booking-request">{{ $sl }}</span>
                  @elseif($b->status==1)<span class="badge-status s-booked">{{ $sl }}</span>
                  @elseif($b->status==2)<span class="badge-status s-issued">{{ $sl }}</span>
                  @elseif($b->status==3)<span class="badge-status s-cancelled">{{ $sl }}</span>
                  @else<span class="badge-status s-ticket-cancelled">{{ $sl }}</span>
                  @endif
                </td>
                <td>BDT {{ number_format($b->total_fare ?? 0, 2) }}</td>
                <td>
                  @if($b->partial_payment)
                    <span class="badge-partial-yes">YES</span>
                  @else
                    <span class="badge-partial-no">NO</span>
                  @endif
                </td>
                <td><a href="{{ url('flight/booking/details/'.$b->booking_no) }}" class="btn-view">View</a></td>
              </tr>
              @empty
              <tr><td colspan="12" class="text-center py-4 text-muted">No bookings found.</td></tr>
              @endforelse
            </tbody>
          </table>
        </div>
        <div class="d-flex justify-content-between align-items-center px-3 py-2">
          <small class="text-muted">Showing {{ $bookings->firstItem() ?? 0 }}–{{ $bookings->lastItem() ?? 0 }} of {{ $bookings->total() }} entries</small>
          {{ $bookings->links() }}
        </div>
      </div>
    </div>
  </div>
</div>
@endsection
