    ...
    <div class="cms-grid">
        @forelse($items as $item)
            <div class="cms-item">
                ...
                @endif
                <h6>{{ $item->title }}</h6>
                <p class="meta mb-1">{{ Str::limit($item->subtitle, 60) }}</p>
                <div class="meta">
    ...
    <div class="modal fade cms-modal" id="addModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ url('cms/banners') }}" method="POST" enctype="multipart/form-data">
                    @csrf
...
@section('footer_js')
    <script>
        function editItem(item) {
            $('#editForm').attr('action', '{{ url("cms/banners") }}/' + item.id);
        ...
        function deleteItem(id) {
            ...
            $.ajax({ url: '{{ url("cms/banners") }}/' + id, type: 'DELETE', data: { _token: '{{ csrf_token() }}' }, success: () => location.reload() });
        }
    </script>
@endsection